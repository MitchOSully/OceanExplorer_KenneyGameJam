...
namespace TarodevController
{
    ...
    /// </summary>
    public class CJoePlayerController : MonoBehaviour, IPlayerController
    {
        ...
        private void Update()
        {
            ...
            CalculateXMovement(); //Horizontal movement (walk if on land, paddle if in water. Same thing from code's pov)
            if (m_bUnderwater)
            {
                CalculateYMovement(); //Vertical paddle movement
            }
            else //land
            { 
        ...
        private void GatherInput()
        {
            if (m_bPhoneControlsActive)
            {
                ...
                {
                    JumpDown = m_phoneControls.m_bJumpDown, //Land-only control
                    JumpUp = m_phoneControls.m_bJumpUp, //Land-only control
                    X = m_phoneControls.m_moveVector.x,
                    Y = m_phoneControls.m_moveVector.y //Water-only control
                };
            }
            else
            {
                Input = new FrameInput
                {
                    JumpDown = UnityEngine.Input.GetButtonDown("Jump"), //Land-only control
                    JumpUp = UnityEngine.Input.GetButtonUp("Jump"), //Land-only control
                    X = UnityEngine.Input.GetAxisRaw("Horizontal"),
                    Y = UnityEngine.Input.GetAxisRaw("Vertical") //Water-only control
                };
            }
        ...
        [Header("WALKING/PADDLING")][SerializeField] private float _acceleration = 90;
        ...
        public void ActivateLandControls()
        {
            m_bUnderwater = false;
            //Walk/Paddle
            _acceleration = 90;
        ...
        public void ActivateWaterControls()
        {
            m_bUnderwater = true;
            //Walk/Paddle
            _acceleration = 10;
