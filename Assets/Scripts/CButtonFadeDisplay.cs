...
class CButtonFadeDisplay : MonoBehaviour
{
    ...
    private IEnumerator DisplayRoutine(string message)
    {
        ...
        text = messageButton.GetComponentInChildren<TextMeshProUGUI>(); //Button text
        OGTextColor = text.color; //Save original text colour

        text.SetText(message); //Set button text to message
